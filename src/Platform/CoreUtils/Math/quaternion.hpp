...
public:
    ...
    Vec3 getVector() const { return Vec3(x, y, z); }
    ...
    Quaternion operator*(Vec3 const& v) const {
        return Quaternion(-w * v.x - x * v.y - y * v.z, w * v.x + x * v.y + y * v.z,
                          w * v.y - x * v.x + y * v.z, w * v.z + x * v.x - y * v.y);
    }
