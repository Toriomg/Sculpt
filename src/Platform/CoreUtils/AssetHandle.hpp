// Typed 64-bit asset handle with a static ID factory; used as a key into AssetRegistry by
// AssetManager.
#pragma once
...
struct AssetHandle {
    ...
    bool operator==(AssetHandle const& other) const { return ID == other.ID; }
    bool operator!=(AssetHandle const& other) const { return ID != other.ID; }
...
namespace std {
    ...
    template <> struct hash<AssetHandle> {
        size_t operator()(AssetHandle const& handle) const { return hash<uint64_t>()(handle.ID); }
    };
