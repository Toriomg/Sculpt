...
PickingTexture::PickingTexture(uint32_t width, uint32_t height) : m_Width(width), m_Height(height) {
    Invalidate();
}
...
PickingResult PickingTexture::ReadPixel(uint32_t x, uint32_t y) const {
    ...
    CORE_LOG_TRACE("ReadPixel: viewport({},{}) -> gl({},{}) | FBO={}x{}", x, y, x, yGL, m_Width,
                   m_Height);
    ...
    // id 0 means the pixel was cleared (no entity). Valid IDs were stored as entityID+1 by
    // PickingSystem.
    if (idData[0] == 0u) {
        result.ObjectID = 0;
        result.Valid    = false;
