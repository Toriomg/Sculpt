...
Shader::Shader(std::string_view vertFilepath, std::string_view fragFilepath)
    ...
    std::string const vertexSource   = ReadFile(std::string(vertFilepath));
    std::string const fragmentSource = ReadFile(std::string(fragFilepath));
...
ShaderProgramSource Shader::ParseShader(std::string_view filepath) {
    ...
    while (getline(stream, line)) {
        if (line.contains("#shader")) {  // Check if the line contains a shader directive
            ...
            if (type != ShaderType::NONE) { ss[static_cast<int>(type)] << line << '\n'; }
        }
    }
    ...
    return {ss[0].str(), ss[1].str()};  // Return the vertex and fragment shader sources as a
...
unsigned int Shader::CompileShader(std::string const& source, unsigned int type) {
    ...
    int result = 0;
    GLCall(glGetShaderiv(id, GL_COMPILE_STATUS,
                         &result));  // Check if the shader compiled successfully
    if (result == GL_FALSE) {        // If compilation failed
        int length = 0;
        GLCall(glGetShaderiv(id, GL_INFO_LOG_LENGTH, &length));

        std::vector<char> message(length);  // <-- If length is 0, vector is empty
        GLCall(glGetShaderInfoLog(id, length, &length,
                                  message.data()));  // <-- message.data() might be null
...
void Shader::SetUniform1ui(std::string const& name, unsigned int value) {
    GLCall(glUniform1ui(GetUniformLocation(name), value));
}

void Shader::SetUniform1iv(std::string const& name, int const* array, int size) {
...
void Shader::SetUniformMat4f(std::string const& name, Matx4f const& matrix) {
