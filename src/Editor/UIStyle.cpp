...
static constexpr ImVec4 rgba(float r, float g, float b, float a = 1.0f) {
    return {r, g, b, a};
}

// ─── shared shape / layout tweaks (same for all themes) ─────────────────────

static void applyShapeDefaults(ImGuiStyle& s) {
...
static void applyModelingLight() {
